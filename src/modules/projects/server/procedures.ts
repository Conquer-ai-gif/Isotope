...
export const projectsRouter = createTRPCRouter({
  ...
  pushToGitHub: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { commitSha, branch } = await pushFragmentToGitHub({
        ...
        commitMessage: `feat: ${fragment.title} — synced from Luno`,
      });
      return {
        commitSha, branch,
      ...
      if (input.createRepo) {
        try {
          await octokit.repos.createForAuthenticatedUser({
            ...
            private: true,
            auto_init: true,
            description: `Generated by Luno — ${project.name}`,
          });
        } catch (e: any) {
  ...
  resolveConflict: protectedProcedure
    .input(z.object({ id: z.string().min(1), resolvedBy: z.enum(['luno', 'github']) }))
    .mutation(async ({ input, ctx }) => {
  ...
  // ── Toggle "Built with Luno" badge (Pro only) ─────────────────────────────
